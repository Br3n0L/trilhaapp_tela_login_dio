...
class AppStorageService {
  Future<void> setDadosCadastraisNomes(String nome) async {
    await _setString(
  ...
  Future<void> setDadosCadastraisDataNascimento(DateTime data) async {
    await _setString(
        STORAGE_CHAVES.CHAVE_DADOS_CADASTRAIS_DATA_NACISMENTO.toString(),
        data.toString());
  }
  ...
  Future<List<String>> getDadosCadastraisLinguagens() async {
    return _getStringStringList(
        STORAGE_CHAVES.CHAVE_DADOS_CADASTRAIS_LINGUAGENS.toString());
  }

  Future<void> setDadosCadastraisTempoDeExperiencia(int value) async {
    await _setInt(
        STORAGE_CHAVES.CHAVE_DADOS_CADASTRAIS_TEMPO_EXPERIENCIA.toString(),
        value);
  }

  Future<int> getDadosCadastraisTempoDeExperiencia() async {
    return _getInt(
        STORAGE_CHAVES.CHAVE_DADOS_CADASTRAIS_TEMPO_EXPERIENCIA.toString());
  }
  ...
  Future<void> setConfiguracoesPageNomeDoUsuario(String nome) async {
    await _setString(STORAGE_CHAVES.CHAVE_NOME_USUARIO.toString(), nome);
  }

  Future<String> getConfiguracoesPageNomeDoUsuario() async {
  ...
  Future<void> setConfiguracoesReceberNotificacoes(bool value) async {
    await _setBool(STORAGE_CHAVES.CHAVE_RECEBER_NOTIFICACOES.toString(), false);
  }
  ...
  Future<void> setConfiguracoesModoEscuro(bool value) async {
    await _setBool(STORAGE_CHAVES.CHAVE_MODO_ESCURO.toString(), false);
  }
...
//----------------------------------------------------------------------------------//
  ...
  Future<List<String>> _getStringStringList(String chave) async {
    var storage = await SharedPreferences.getInstance();
